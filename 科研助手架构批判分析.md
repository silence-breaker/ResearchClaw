...
来源：`科研方法论.md`、手绘草图、`oh-my-claude-code` / OpenClaw 现有实现、OpenClaw-first 技术路线讨论。

## 一句话结论

为了尽快做出 demo，技术路线应改为 `OpenClaw-first`：OpenClaw 负责入口和接口层，你自己的 Research Workflow Engine 负责科研状态机，OMC 暂时从核心链路移除。
...
2. `Critic` 独立出来是对的。
   但它应该是 workflow 里的评审节点，而不是必须依赖 OMC 的 critic agent。
...
6. 旧方案里 OpenClaw、OMC、Research Workflow Engine 有调度职责重叠。
   OpenClaw 有 gateway / agent runtime 方向，OMC 有 team runtime / task router，你自己又有科研状态机。
   三套调度中心叠在一起，会让状态同步、部署稳定性和责任归属都变复杂。

7. MVP 阶段不该先接 OMC。
   当前最重要的是跑出 demo，而不是先搭一个完整多 agent team runtime。
   如果科研任务暂时不需要多个隔离 worktree 并行，OMC 会带来额外状态、tmux、CLI、版本和部署风险。
...
- Model Adapters：Claude、Gemini、Codex 的直接适配层，MVP 阶段不经过 OMC。
- Evidence Store：raw log、artifact、code summary、research memory、实验结果。
- OMC：暂时不进入 MVP 核心链路，只作为设计参考或后续可选 adapter。

## 为什么 MVP 先不接 OMC

OMC 不是坏东西，它是一个多 agent 编排系统和运行时底座。
它提供 team runtime、task router、worker、summary、memory、agent prompt 等能力。

但对当前 demo 来说，问题不是“OMC 能不能做”，而是“现在接它会不会让架构变重”。

旧方案里会同时出现三套状态：

- OpenClaw 的 session / hook event / gateway signal
- Research Workflow Engine 的 phase / contract / evidence state
- OMC 的 team task / worker / inbox / outbox / summary

这会带来几个直接风险：

- single source of truth 不清楚
- 服务器部署依赖变重，容易卡在 tmux、CLI、worker 生命周期
- debug 时不知道问题在 OpenClaw、workflow engine 还是 OMC
- demo 目标会被运行时集成复杂度拖慢
...
- `Model Adapters`：直接调用 Claude、Gemini、Codex，不在 MVP 阶段经过 OMC。
...
1. 设计参考。
   它的 planner、critic、verifier、summary、memory、team task 思路可以借鉴。

2. 可选 adapter。
   如果后面真的需要多个 worker 并行、隔离 worktree、后台 team runtime，可以再加 `omcAdapter`。

3. 非 MVP 依赖。
   第一版 demo 不把 OMC 放进核心链路，不让它参与状态推进。
...
flowchart TB
  ...
  OMCO[OMC optional later<br/>parallel team runtime] -. future adapter .-> RWE
```
...
6. OMC、并行 worker、多 agent team runtime 放到 demo 之后。

## 不建议一开始就做的事

- 多模型互相投票。
- 让两个 agent 互相 review 到没完没了。
- 过早接入 OMC team runtime。
